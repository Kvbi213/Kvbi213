...
| **Standard Ikonograficzny** | Lokalne wektory SVG w `assets/icons/` | Stabilność renderowania offline i online bez zależności od zewnętrznych CDN-ów |
...
| **05. CORE DEPLOYS** | Featured Systems + Live Pin | GitHub Stats Repo Pin API (`eight-theta` verified) + Wektorowe tabele | Prezentacja `ai-system-dashboard` (OmniDash) i Portfolio OS |
| **06. SYSTEM INTELLIGENCE** | Wektory Operacyjne | Tabele Markdown + Lokalne wektory SVG (`assets/icons/`) | 4 filary inżynierskie |
...
| **08. SYSTEM METRICS** | GitHub Analytics | Readme-Stats (`eight-theta`) & DenverCoder1 Streak Stats API | Wskaźniki commitów, języki, streak (monochrome) |
| **09. TERMINAL FOOTER** | System Dispatch Log | Monospace Code Block + `assets/icons/terminal.svg` | Sygnatura, status zakończenia transmisji |
...
│
├── /docs/
│   ├── /versions/          ← Logi wydań (v1.0.0.md, v1.1.0.md, v1.1.1.md)
│   ├── /architecture/      ← Diagramy architektury
│   └── /errors/            ← Rejestr ERROR_DIFF
│
└── /assets/
    ├── header.svg          ← Autorski wektorowy baner systemowy SVG
    └── /icons/             ← Lokalne, niezależne pliki SVG ikon (git-branch, cpu, globe, bot, etc.)
```
